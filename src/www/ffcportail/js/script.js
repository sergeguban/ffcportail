$(document).ready(function() {
	...
	$('.therapeutics-table').dataTable( {
		...
		"oLanguage": {
			...
			/* for using with dynamic content 
			"bProcessing": true,
        	"bServerSide": true,
        	"sAjaxSource": "../server_processing.php?arg=...."
			*/
        }
	...
	// Reset Font Size on top page 
	 ...
	 // select profesionnal radio buttons 
	 $('#therapeutic-search-pro-form input[type=radio]').click(function() {
		radioId = '';
		radioId = $(this).attr('id'); 
		switch (radioId) {
			case 'therapeutic-search-pro-form-filter-0' :  // nihii
			$('#therapeutic-search-pro-form input[type=text]').removeAttr('disabled');
			$('#therapeutic-search-pro-form-firstname, #therapeutic-search-pro-form-lastname').attr("disabled", "disabled");
			break;
			case 'therapeutic-search-pro-form-filter-1' : // name
			$('#therapeutic-search-pro-form input[type=text]').removeAttr('disabled');
			$('#therapeutic-search-pro-form-nihii').attr("disabled", "disabled");
			break;
		}
	 });
  ...
  if($.browser.msie){
	...
	$('#patient-management form').corner('3px');  
	}
});
