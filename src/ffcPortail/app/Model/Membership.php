...
private $ageCategories = array(12=>0,18=>1,25=>2,35=>3,200=>4);
	...
	public function loadStats($club, $year){
		...
		if($club=='all'){
			...
			$stats['comp_total']['all']=$this->count_comp($db->fetchAll('SELECT User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.licence=? and Membership.l_status=? and (Membership.l_type=? or Membership.l_type=?) and Membership.year=? and User.ffc_id is not null',array(1,'produced','Competition','Disciplines',$year)),$year);
			$stats['comp_total']['men']=$this->count_comp($db->fetchAll('SELECT User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.licence=? and Membership.l_status=? and (Membership.l_type=? or Membership.l_type=?) and Membership.year=? and User.sexe=? and User.ffc_id is not null',array(1,'produced','Competition','Disciplines',$year,'H')),$year);
			$stats['comp_total']['women']=$this->count_comp($db->fetchAll('SELECT User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.licence=? and Membership.l_status=? and (Membership.l_type=? or Membership.l_type=?) and Membership.year=? and User.sexe=? and User.ffc_id is not null',array(1,'produced','Competition','Disciplines',$year,'F')),$year);
			...
			$stats['comp_total']['all']=$this->count_comp($db->fetchAll('SELECT Membership.year,User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.club=? and Membership.licence=? and Membership.l_status=? and (Membership.l_type=? or Membership.l_type=?) and Membership.year=? ',array($club,1,'produced','Competition','Disciplines',$year)),$year);
			$stats['comp_total']['men']=$this->count_comp($db->fetchAll('SELECT Membership.year,User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.club=? and Membership.licence=? and Membership.l_status=? and (Membership.l_type=?  or Membership.l_type=?)and Membership.year=? and User.sexe=?',array($club,1,'produced','Competition','Disciplines',$year,'H')),$year);
			$stats['comp_total']['women']=$this->count_comp($db->fetchAll('SELECT Membership.year,User.date_naissance from memberships as Membership left join users as User on Membership.user_id=User.id where Membership.club=? and Membership.licence=? and Membership.l_status=? and (Membership.l_type=?  or Membership.l_type=?) and Membership.year=? and User.sexe=?',array($club,1,'produced','Competition','Disciplines',$year,'F')),$year);
